...
public class CoinCounter {
    ...
    public int simpleNumberOfCoinsRequired(int totalSum) {
        ...
        for (int i=3 ; i >= 0 ; i--) {
            if (totalSum < denominations[i]) {
                ...
                int coins = 0;
                coins = totalSum / denominations[i];
    ...
    public int numberOfCoinsRequired(int totalSum) {
        ...
        if ( (totalSum == denominations[0]) || (totalSum == denominations[1]) || (totalSum == denominations[2]) || (totalSum == denominations[3]) ) {
            coinsCounter = 1;
        }
            ...
            for (int i=3 ; i >= 0 ; i--) {
                List<Integer> coinOptions = new ArrayList<Integer>();
                Integer newValue = totalSum - denominations[i];  //1, 5, 7, 20
                for (int j=3 ; j >= 0 ; j--) {
                    if (newValue < denominations[j]) {
                        // do nothing
                    }
                    else {
                        int coins = 0;
                        coins = newValue / denominations[j];
                        coinsCounter = coinsCounter + coins;
                        newValue = newValue - (coins * denominations[j]);
                        coinOptions.add(newValue);
                    }
                }
